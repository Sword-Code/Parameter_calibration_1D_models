...
def run_pars_separately(conf):
    mpi.print(f"routine: run_pars_separately, conf: {conf.name}. Start!")
    
    path_observations = conf.path_observations
    model_directory = conf.model_directory_separately
    ...
    n_ens_members = conf.n_ens_members_separately
    
    mpi.print(f"routine: run_pars_separately, conf: {conf.name}. n_ens_members={n_ens_members}.")
    
    # RMSE = np.zeros((n_ens_members, 16))  # the skill score is stored here
    RMSE = {par:[] for par,parameter in enumerate(perturbed_parameters_listed)} # the skill score is stored here
    ...
    for par,parameter in enumerate(perturbed_parameters_listed):

        for member in range(1,n_ens_members+1):
            if mpi.rank != int((member-1+par*n_ens_members) * mpi.size / (len(perturbed_parameters_listed)*n_ens_members)):
                continue
        
            print(parameter, member, flush=True)

            calibration_init = cm.calibrate_model(
                length_period = length_of_data, 
                obs_types = observed_types, 
                path_mod = model_directory.format(parameter)+"/result_"+str(member).zfill(4)+".nc", 
            ...
            RMSE[par].append(calibration_init.RMSE_metric())
    
    root=[]
    RMSEs=[]
    i_am_root=0
    for par,parameter in enumerate(perturbed_parameters_listed):
        root.append(int((int(n_ens_members*0.5)+par*n_ens_members) * mpi.size / (len(perturbed_parameters_listed)*n_ens_members)))
        RMSEs.append(mpi.gather(RMSE[par], root=root[par]))
        if mpi.rank==root[par]:
            i_am_root=1
            
    mpi_splitted=mpi.split(color=i_am_root)
    ...
    for par,parameter in enumerate(perturbed_parameters_listed):
        if mpi.rank != root[par]:
            continue
        RMSE.append(np.array(unroll(RMSEs[par])))
    ...
    # minimize RMSE to find the index of the best performing ensemble member
        ...
        best_ensemble_member = np.argwhere(RMSE[-1]==np.amin(RMSE[-1]))[0][0]+1  # identify the ensemble member number corresponding to minimum RMSE
    ...
    #for parameter in perturbed_parameters_listed:
        with eatpy.models.gotm.YAMLEnsemble(model_directory.format(parameter)+"/fabm_"+str(best_ensemble_member).zfill(4)+".yaml", 1) as fabm:
    ...
    RMSEs=mpi_splitted.gather(RMSE)
    OPVs=mpi_splitted.gather(optimal_parameter_values)
    
    if mpi_splitted.rank != mpi_splitted.root:
        return
    
    RMSE=np.array(unroll(RMSEs))
    for d in OPVs:
        optimal_parameter_values.update(d)

    # np.savetxt("RMSE_all_ind.txt", RMSE)
    np.savetxt(conf.save_dir/f"{conf.name}_RMSE_all_ind.txt", RMSE)
    
    with open(conf.save_dir/f"{conf.name}_best_parameters_individually.csv", "w") as csv_file:
...
def main():
    run_pars_separately(chosen_conf)
